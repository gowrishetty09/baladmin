...
const shouldLogSocket =
  __DEV__ &&
  (process.env.EXPO_PUBLIC_DEBUG_WS === "1" ||
    process.env.EXPO_PUBLIC_DEBUG_WS === "true");
...
export const MonitoringScreen: React.FC = () => {
  ...
  const extractLocation = (raw: any): DriverLocation | null => {
    ...
    // react-native-maps can crash natively when given NaN/Infinity or out-of-range coordinates.
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) return null;
    if (lat < -90 || lat > 90) return null;
    if (lng < -180 || lng > 180) return null;

    const heading = raw.heading !== undefined ? Number(raw.heading) : undefined;
    const speed = raw.speed !== undefined ? Number(raw.speed) : undefined;
    ...
    return {
      ...
      bookingId: raw.bookingId
        ...
        : raw.booking?.id
          ? String(raw.booking.id)
          : undefined,
      latitude: lat,
      longitude: lng,
      heading:
        heading !== undefined && Number.isFinite(heading) ? heading : undefined,
  ...
  const connect = () => {
    ...
    const handleAdminFleet = (payload: any) => {
      ...
      if (payload?.type === "snapshot") {
        const driversList = Array.isArray(payload?.drivers)
          ? payload.drivers
          : [];
  ...
  const visibleKey = useMemo(() => {
    return visibleDrivers
      .map(
        (d) =>
          `${d.driverId}:${d.latitude.toFixed(5)},${d.longitude.toFixed(5)}`,
      )
      .join("|");
  }, [visibleDrivers]);
      ...
      <View style={[styles.headerContainer, { paddingTop: insets.top }]}>
        <View
          style={[
            ...
            { backgroundColor: Colors.navy + "F5" },
          ]}
        />
